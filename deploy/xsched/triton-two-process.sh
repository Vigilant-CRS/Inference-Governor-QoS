...
shim_env() { # Prioritaet Schwelle Batch Implementierung
  local impl=""
  [ "$4" != "TRAP" ] && impl="XSCHED_CUDA_LV3_IMPL=$4"
  echo "export XSCHED_SCHEDULER=GLB XSCHED_AUTO_XQUEUE=ON XSCHED_AUTO_XQUEUE_LEVEL=${LEVEL:-2} \
...
case "${1:-}" in
  up)
    ...
    if [ "$mode" = xsched ]; then
      ...
      sleep 2
      pre_a=$(shim_env 1 "${THRESH_A:-16}" "${BATCH_A:-8}" "$impl")
      pre_b=$(shim_env 0 "${THRESH_B:-4}" "${BATCH_B:-2}" "$impl")
    fi
    ...
    wait_ready 9200 && wait_ready 9100
    echo "bereit: $mode $impl"
    ;;
  down)
    docker rm -f xs-server xs-triton-a xs-triton-b >/dev/null 2>&1 || true
    ;;
