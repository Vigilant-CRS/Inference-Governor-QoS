...
Stand: 2026-09-15 · **Gate M3 bestanden, auf Treiber 580.178.04 bestätigt** ·
...
| Tests | 922 grün in 61 Suiten, sechs bewusst ignoriert (die Zeitprüfung der Datenpfadbudgets, das große NV-23-Gitter, zwei Messmatrizen der Margenkalibrierung, die Tabelle der Lastspitzen-Analyse und die des Versorgungsschutzes — sie gehören auf die Releasemaschine, nicht auf geteilte CI-Runner) |
| Architekturentscheidungen | 46 ADRs |
...
| Externer Review vom 14.09. | **alle Codebefunde behoben** (R01–R04, R06) | R01/R02: der Look-ahead prognostiziert die freigegebene Variante samt Vorverarbeitung, und die Verweildauer blockiert keine Rettung der Versorgung. R03/R04 ([ADR-0042](adr/0042-an-end-is-proven-not-assumed.md)): ein Messarm endet erst, wenn keine seiner Inferenzen mehr laeuft, die Quarantaene einer Region gilt prozessweit, und ein Zaehlerabfall beendet nur Aufrufe, die das Backend in dieser Epoche gezaehlt hat. R06: das Basisalter eines Lageberichts nennt die aelteste Kamera (`pilot.rs`, am 15.09. im Code nachgeprueft). R05/R07/R08 sind Einordnung, kein Codefehler ([Review](reviews/2026-09-14/REVIEW.md)). **Offen und im Betrieb spuerbar:** nach einem echten Absturz des Backends startet der Poller oft erst, wenn der neue Prozess antwortet; dann liest er schon den niedrigen Zaehler, und ein Aufruf, der im alten Prozess lief, bleibt bis zum Neustart des Governors gesperrt. Das ist die in ADR-0042 benannte Kosten („verliert nach und nach Kapazitaet"); abhelfen wuerde eine Boot-Kennung des Backends oder ein Poller, der laeuft, solange ein Aufruf offen ist. |
| `vig autotune` tuned den Governor (ADR-0045) | **gebaut, auf Hardware gelaufen** | Neuer Schritt `tune` zwischen `measure` und `fit`: probiert Pipelining, Versorgungsschutz, gelernte Marge und Sicherheitsmarge in einem Durchgang gegen die Vertraege und behaelt nur, was die geschuetzten Stroeme ueber die Rauschschwelle hinaus besser versorgt; Vertraege, Modelle und Slots bleiben unberuehrt. **Laptop, Gate-M3-Last:** die von Hand abgestimmte Konfiguration war schon die beste von sechs (geschuetzt 0–3 ‰ in jeder Fassung). **Pixel 2, Overload-Vertraege:** bei zwei Slots nicht gesaettigt, nichts zu tunen. Der erste Lauf deckte einen Fehler in der Zielgroesse auf — ein unerfuellbarer nachrangiger Strom (1000 ‰) verdeckte als Maximum jede Verbesserung —, behoben durch den Mittelwert (`08cf0ff`). **Pixel 2, Heavy-Vertraege:** das Tuning behielt zwei Einstellungen (geschuetzt 220 → 50 ‰), doch der Bestaetigungslauf derselben Konfiguration mass bei 125 % Last 440 statt 40 ‰ — der Gewinn liegt in der Streuung eines 10-Sekunden-Fensters und ist **nicht belegt**; ausserdem sind die Vertraege nicht planbar (`vig doctor` NOT_READY, 141 %). Daraus zwei Fehler im Werkzeug, **behoben** (`cbbc79e`): keine Suche bei NOT_READY, und uebernommen wird nur, was in einer abwechselnden Bestaetigung (ungetunt/getunt, zwei Paare) haelt. **Pixel 2, gesaettigt und planbar** (`vig-slots2-saturated.yaml`, 95 %): im 10-s-Fenster verglich die Suche null gegen einen Takt (27 Takte je Punkt); die Bestaetigung verwarf die Marge. Daraus **Fenster in Takten** (`90f517c`, mindestens 200 Takte des langsamsten geschuetzten Stroms) und eine Schwelle aus gezaehlten Takten. **Nachmessung als Anwendungsfall Lieferroboter auf Pixel 2 und Pixel 5** ([Use Cases](use-cases.md)): Detektor bei 90 % Last direkt 293 → 99 ‰ unter dem Governor (Pixel 5: 497 → 208 ‰), bei 100–125 % beide Wege ≤ 4 ‰ — ausser Pixel 5 bei 125 %, dort Governor 32 gegen 3 ‰ (gegen uns); Preis bei `pose` und `depth`. Warum der direkte Weg fast nur am 90-%-Punkt verliert, ist **nicht erklaert**. Tuning behielt auf beiden Telefonen nichts. **Offen:** ein bestaetigter Tuning-Gewinn — auf keiner gemessenen Last hat eine andere Einstellung zweimal gehalten ([Validierung](benchmark/validierung-autotune.md)). |
| Review von `vig autotune` vom 15.09. | **behoben**, ein Rest offen | `vig-fit` wird mit ausgeliefert; Fremdlast ist fremde CPU-Zeit statt `loadavg`; eine Fortsetzung ersetzt Schritteintraege und misst Abhaengiges nach; ein verweigerter Lauf beginnt mit der Verweigerung; `fit` nimmt nur ein frisches, schluessiges, englisches Urteil; ungemessene Modelle zaehlen; die Belegungsstufe erkennt eine Warteschlange und rueckt nach einer verworfenen Stufe nicht nach ([CHANGELOG](../CHANGELOG.md)). **Offen:** ein uebernommenes Soloprofil ist in `measured.yaml` nicht als ungemessen markiert — der Bericht sagt es, die Datei nicht (braucht ein Feld im Manifest). Der Abgleichtest, der zwei- bis dreimal von zehn scheiterte, war ein Wettlauf im Test, nicht im Abgleich. **Nachweis auf Hardware, 15.09.:** Auf dem Pixel 2 erstmals vollstaendig und unverschmutzt (12 von 12 Reihen, Freigabe „not issued") und mit derselben Struktur wie die Handkonfiguration — dasselbe `no_corun`-Paar, dieselben zwei Interferenzeintraege (−6,4 % / +4,5 %), Soloprofile innerhalb von 3,5 %; eine Laststufe des Detektors weicht ab, in der vorsichtigen Richtung. Auf dem Laptop verweigert der Bericht jetzt vorn. Ein zweiter Pixel-2-Lauf eine Stunde spaeter wiederholt die Struktur exakt, die Soloprofile innerhalb von 3,2 %; am meisten streut der kleinere Interferenzaufschlag (+23,5 %). Auf einem **Pixel 5** (zweites SoC, keine Handkonfiguration) ebenfalls vollstaendig und unverschmutzt (12 von 12, „not issued") und mit derselben Struktur wie auf dem Pixel 2 — mit eigenen, langsameren Zahlen; die Tiefe lag dort mit 1,82x zweimal knapp ueber der Warteschlangen-Grenze von 1,80x ([Vergleich](benchmark/validierung-autotune.md#vergleichslauf-am-15092026-mit-dem-reparierten-stand)). |
...
| Mindestlaufzeit für nachrangige Arbeit | **gebaut, im Simulator belegt**, opt-in ([ADR-0046](adr/0046-a-minimum-runtime-is-paid-by-lower-classes.md)) | `contract.min_runtime: { budget_ms, window_ms }` an `normal`/`best_effort`: mit verbleibendem Budget im gleitenden Fenster über `normal`, unter `high`; der Look-ahead gilt unverändert. Anlass war die Demo vom 15.09. (Sprachmodell zwei Antworten in 40 s). Im Simulator mit derselben Form und 300 ms je Sekunde: Sprachmodell 362–384 statt 11 ms Rechenzeit je Sekunde, Front weiter 0 ‰ nicht frisch und ohne verpasste Deadline, die drei `normal`-Kameras zahlen (86–88 → 287–299 ‰ nicht frisch), die Front bekommt seltener in jedem 33-ms-Fenster etwas Neues (104–108 → 155–164 ‰). **Offen:** die Messung auf der GPU (dort ist die Laufzeit neben dem Sprachmodell länger als das Alleinprofil); das Budget schafft keine Lücke für einen unteilbaren Block neben geschützter Arbeit und reserviert keinen Slot. |
| Stetige Zusagen je Strom | **gebaut, auf Hardware gemessen**, opt-in ([ADR-0047](adr/0047-a-goal-has-two-halves-rate-and-gap.md)) | `contract.objective: { coverage_permille, window_ms, max_gap_ms }`: beide Hälften werden in dieselbe Größe umgerechnet — Zeit bis zum Bruch —, die engere entscheidet, geordnet wird **innerhalb** der Klasse und nie darüber (NV-24). Ohne Zusage ist der Ablaufplan bitgleich (27 goldene Tests). Auf dem Laptop hält eine Zusage von 800 ‰ vollständig, solange kein Aufruf länger rechnet als sie selbst; genau den lehnt die Blockierprüfung ab. **Zwei Punkte, die hier als offen standen, sind am 16.09. beantwortet und bewusst nicht gebaut:** Dämpfung — zwei gleichrangige Kameras, identische Zusage, ein Slot zu 100 % belegt, drei Läufe je Arm, Schwelle vorab festgelegt: die Wertebereiche überlappen, kein Effekt; und die Zulassungsrechnung, die nur Ströme **mit** Zusage zählt — 78 % eines Slots tatsächlich gefordert gegen 25 % ausgewiesen, die Zusage hielt trotzdem zu 1000 ‰, weil ein Strom ohne Zusage `latest` fährt und überholte Bilder verwirft; seine Periode ist eine Wunschrate, kein Anspruch. Die Summe zu erweitern hätte drei laufende Demos abgelehnt. **Offen:** nur auf einer Maschine gemessen, und zwei identische Referenzläufe streuten um 100 ‰. |
| Halten statt verwerfen | **entsperrt und gemessen am 16.09.** ([ADR-0014](adr/0014-cooperative-quanta.md), [ADR-0035 Punkt 5a](adr/0035-preemption-is-a-measured-backend-property.md)) | Die Zerlegung stand seit dem 01.09. im Kern und war in **keiner** ausgelieferten Konfiguration eingeschaltet — weil sie sich nicht einschalten ließ: die Blockierprüfung empfahl `cooperative:` und fragte nie, ob ein Modell zerlegbar ist, also kam derselbe Befund zurück und der Governor verweigerte den Start. Auf einer Ausführungseinheit, dem Fall aus ADR-0014, war sie unbenutzbar; auf zwei Slots schweigt die Regel ohnehin, deshalb fiel es nie auf. Behoben: gerechnet wird mit dem **spätesten** Quantum bei vollem Kontext, nicht mit dem ganzen Auftrag. Erstes ausgeliefertes Beispiel `examples/cooperative_llm/`, alle vier Vertragswerte doppelt gemessen (Tokenreihe gegen `vig calibrate`: 6100 gegen 6277 µs Sockel, 256 gegen 258 Token/s). **Gemessen, 30 s je Arm, zwei Läufe je Punkt:** ohne Zerlegung 608 Abweisungen wegen unhaltbarer Frist, mit Zerlegung praktisch keine — und die Kamera liefert rund 908 statt 804 von 910 Bildern, bei gleichem Textausstoß. Auf **einem** Slot trägt es ebenfalls (52 Antworten, keine Abweisung), aber die Kamera reißt ihre Zusage von 100 ms — gemessen 213 bis 236 ms längste Lücke, und zwar auch dann, wenn das Sprachmodell nur alle drei Sekunden gefragt wird (59 % Auslastung, also **kein** Artefakt eines hämmernden Testclients). Die Zähler nennen die Ursache: während **eines** Auftrags werden sechs Kamerabilder überholt, 6 × 33 ms plus Laufzeit ergeben genau die Lücke. Die Zerlegung erkauft einen funktionierenden Dienst, die zweite Ausführungseinheit erkauft die Zusage — deshalb steht das ausgelieferte Beispiel auf zwei Slots (48/57 ms Lücke, 1 ‰ unabgedeckt). Eine Vorlage soll die Zusage halten, die sie ausspricht. **`min_tokens` ist seit dem 16.09. gemessen** und steht im Beispiel auf 4 statt auf dem Vorgabewert 8: auf einem Slot kostet 8 **160 geschützte Kamerabilder** (743/764 statt 905/905) und kauft dafür Text — der falsche Tausch für einen `protected`-Strom; 16 lässt den Auftrag **kollabieren** (zwei Antworten, GPU nur halb ausgelastet), weil das kleinste Quantum nicht mehr in die Lücke passt und der Look-ahead jede Fortsetzung vetoiert. Auf **zwei** Slots bewegt dieselbe Reihe fast nichts — die Quantengröße ist nur bei knapper Kapazität der Engpass. **Offen:** der Wert wird von Hand gesetzt. `vig autotune` könnte ihn suchen, aber seine Schutzregel `untouched()` verweigert jede Fassung, die etwas unterhalb von `models.` ändert — ein Einbau wäre eine Aufweichung dieser Zusage und braucht eine Entscheidung, keine Zeile Code. |
| Blockierprüfung zählt Modelle, nicht gleichzeitige Aufträge | **offen, Befund belegt** (Recheck 16.09., Punkt 1). `SlotSet::ready_slot` sucht den ersten Slot, der ein Modell zulässt und noch Kredit hat — eine Grenze „ein Modell, ein Slot" gibt es im Code nicht. Ein Modell mit `capacity: 2` kann damit zwei Slots belegen, während `check_blocking_work` es als **einen** Blocker zählt. **Die naheliegende Verschärfung ist gebaut und wieder zurückgenommen worden:** rechnet man je Blocker `capacity + 1` Slots, lehnt die Prüfung die veröffentlichte Demo ab (`slots: 2`, Sprachmodell `capacity: 2`) — die nachweislich läuft. Die theoretische Obergrenze trifft die Wirklichkeit nicht, weil der Governor einen zweiten Auftrag eines `best_effort`-Modells nur dispatcht, wenn nichts Wichtigeres wartet, und der Look-ahead ihn vor einer geschützten Ankunft zurückhält. Der tragfähige Weg ist der andere, den der Review nennt: eine **durchgesetzte** Parallelitätsgrenze im Kern, damit die Annahme der Zulassung stimmt statt nur unterstellt zu sein. **Der Nachweis ist seit dem 16.09. erbracht:** `one_model_can_occupy_two_slots` in `crates/vig-core/tests/scheduler_flow.rs` zeigt es ohne Pipelining — zwei Aufträge eines `best_effort`-Modells landen in zwei verschiedenen Slots. Der Befund ist damit real, nicht theoretisch. Was er **nicht** zeigt, ist Schaden: dieselbe Konstellation fährt die veröffentlichte Demo, die trägt. Die Durchsetzung im Kern muss deren Zahlen deshalb unberührt lassen, und das entscheidet eine Messung, keine Regel. |
...
**16.09.2026, RTX 4070 Laptop, intern.** Vier Reihen an einem Tag: Mischlast
aus Detektor, Whisper und Sprachmodell über einen Governor; Zusagen auf
Hardware (800 ‰ gehalten, gebrochen allein durch einen Aufruf, der länger
rechnet als die Zusage); die Zerlegung entsperrt und gegengemessen (608
Abweisungen gegen null, 908 statt 804 Bilder); Dämpfung und Zulassungslücke
geprüft und **nicht** gebaut, mit vorab festgelegter Schwelle und drei Läufen
je Arm. Methodisch festgehalten, weil es teurer war als die Zahlen: zwei
Versuche dieses Tages hätten beinahe einen Effekt gemeldet, den ein
