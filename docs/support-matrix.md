...
| Cooperative decomposition of generative jobs | **Qualified** | `cooperative:` on the model. Until 16 September switching it on did not help on a **single** execution unit: the admission check that recommends decomposition never asked whether a model was decomposable, so the finding came back and the governor refused to start ([ADR-0035, point 5a](adr/0035-preemption-is-a-measured-backend-property.md)). Fixed, together with the first shipped example that uses it ([`examples/cooperative_llm/`](../examples/cooperative_llm/vig.yaml)) |
...
| Minimum runtime for lower-priority work | Reachable | `min_runtime: { budget_ms, window_ms }` on the contract of a `normal` or `best_effort` model ([ADR-0046](adr/0046-a-minimum-runtime-is-paid-by-lower-classes.md)). While the budget in the sliding window is not used up, its waiting work ranks above `normal` and below `high`; paid by the lower classes, never by guarded work — the look-ahead still holds it back. It cannot create a gap a non-interruptible block needs next to protected work. `vig doctor` adds the budgets to the protected utilization (`RUNTIME_BUDGET_UNSCHEDULABLE`). Reproduced in the simulator, not yet measured on the GPU |
