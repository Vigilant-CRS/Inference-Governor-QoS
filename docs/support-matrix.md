...
| Preemptible background lane | Reachable | `backend.preemptible_lanes` plus `preemptible: { residual_blocking_us, source }` on a model served by its own low-priority backend process ([ADR-0035](adr/0035-preemption-is-a-measured-backend-property.md)). Needs a preemption layer in the backend such as [XSched](../deploy/xsched/README.md); the residual blocking R must be measured with `vig calibrate`, and `vig doctor` warns while it is declared. Not measured on the GPU yet |
