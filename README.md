...
| Open | Why it matters |
|---|---|
| Cooperative decomposition was switched on nowhere — because it could not be | In the core and measured since 1 September, yet until 16 September **no shipped configuration set `cooperative:`**. The reason was a contradiction in our own tool: the admission check whose finding recommends decomposition never asked whether a model *was* decomposable. Following its advice produced the same finding again, and the governor refused to start — so on one execution unit, the case ADR-0014 exists for, the capability was locked shut. Fixed ([ADR-0035, point 5a](docs/adr/0035-preemption-is-a-measured-backend-property.md)), and the first example that switches it on is [`examples/cooperative_llm/`](examples/cooperative_llm/vig.yaml). What a full day of mixed-load measurement concluded from the indivisible block ("more slots, or a shorter call") was wrong for the same reason. |
| Continuous objectives are measured on one machine only | `objective:` holds on this laptop: 800 ‰ promised, 685–736 ‰ reached beside a long neighbour, 930–1000 ‰ without one. Two identical reference runs differed by 100 ‰, so single runs scatter. Damping against oscillation is **not built, and we looked for the reason to build it**: two equally ranked streams with identical promises, one slot, slots 100 % busy, three runs per arm against a threshold fixed beforehand — the ranges overlap and the values *with* promises are if anything lower ([ADR-0047](docs/adr/0047-a-goal-has-two-halves-rate-and-gap.md)). A tie cannot oscillate anyway: the candidate key is fully ordered down to the model index. Admission also counts only streams that carry a promise; one without it is a wish, not a claim, and its queue drops superseded frames. Measured: 78 % of a slot actually demanded against 25 % declared, and the promise still held at 1000 ‰. Deliberate — but the number is not total demand, and that now says so. |
