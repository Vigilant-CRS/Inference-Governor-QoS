...
async fn run() {
    ...
    for (logical, physical, _, _) in BASE {
        ...
        let region = Region::create(&format!("vig_diy_{logical}"), byte_size).expect("Shm-Region");
        let _ = triton
            .raw()
            .await
            .expect("Backend")
    ...
    println!(
        "  Last | Betriebsart                     | Detektor | alle Stroeme | Antwortalter p95"
    );
    println!("  -----|---------------------------------|----------|--------------|--------");
    ...
    for load in LOADS {
        ...
        for arm in [Arm::Naiv, Arm::Eigenbau, Arm::Governor] {
            ...
            for _ in 0..REPEATS {
                ...
                aoi.push(worst_response_age_ms(&reports));
            }
