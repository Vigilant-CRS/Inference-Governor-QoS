...
async fn build_streams(resolved: &onetimer_config::schema::Resolved) -> Vec<StreamDef> {
    let probe = onetimer_backend_triton::TritonClient::new(&resolved.backend_endpoint);
...
async fn run() {
    ...
    let backend = Arc::new(onetimer_backend_triton::TritonClient::new(
