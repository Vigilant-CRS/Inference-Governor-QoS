...
use onetimer_bench::workload::connect;
use onetimer_config::Config;
use onetimer_gateway::cooperative::{SAMPLING_PARAMETERS, TEXT_INPUT, read_text_output};
...
use onetimer_sim::coverage::CoverageTracker;
...
async fn measure(
    ...
    let tracker = Arc::new(Mutex::new(CoverageTracker::new(
        onetimer_core::Duration::from_nanos_unbounded(DETECTOR_PERIOD_MS * 1_000_000),
        onetimer_core::Duration::from_nanos_unbounded(DETECTOR_MAX_AGE_MS * 1_000_000),
        onetimer_core::Instant::ZERO,
        onetimer_core::Duration::from_nanos_unbounded(RUN_SECONDS * 1_000_000_000),
    )));
    ...
    //   ueber den Governor -> unaer, denn genau das bietet OneTimer an
    //
    // Beides fuehrt zur selben Arbeit auf demselben Backend; OneTimer
    ...
    let generator = {
        let direct = onetimer_backend_triton::TritonClient::new(llm_endpoint);
    ...
    let coverage = tracker.lock().map_or(
        onetimer_sim::coverage::Coverage {
            covered: 0,
            total: 0,
            aoi_p50_ns: 0,
            aoi_p95_ns: 0,
    ...
    Outcome {
        ...
        detector_aoi_p95_ms: coverage.aoi_p95_ns / 1_000_000,
        generations_finished: finished.load(Ordering::Relaxed),
        generated_chars: chars.load(Ordering::Relaxed),
    }
}

async fn start_gateway(config: &Config) -> (String, onetimer_gateway::Handle) {
...
fn config_yaml(cooperative: bool) -> String {
    let block = if cooperative {
        "\n    cooperative: { tokens_per_second: 55, min_tokens: 4, max_total_tokens: 64 }"
    } else {
        ""
    };
    format!(
...
fn report(outcomes: &[Outcome]) {
    ...
    for o in outcomes {
        ...
        println!(
            ...
            o.label, o.detector_aoi_p95_ms, o.generations_finished, o.generated_chars
        );
    }
}
...
async fn probe_quantum_cost() {
    let client = onetimer_backend_triton::TritonClient::new("127.0.0.1:8011");
...
async fn run() {
    ...
    let triton = onetimer_backend_triton::TritonClient::new("127.0.0.1:8001");
    let metadata = triton
        .raw()
        .await
    ...
    let region = Region::create("onetimer_wp26", byte_size).expect("Shm-Region");
    let _ = triton
        .raw()
        .await
        .expect("Backend")
