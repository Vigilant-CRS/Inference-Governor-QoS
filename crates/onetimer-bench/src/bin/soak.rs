...
use onetimer_bench::workload::{InputSpec, StreamDef, drive};
...
async fn start_gateway(yaml: &str, metrics_port: u16) -> String {
    ...
    tokio::spawn(async move {
        ...
        let _ = onetimer_gateway::exporter::serve(metrics_handle, address).await;
    });
...
async fn run() {
    ...
    for (logical, physical, _, _) in BASE {
        ...
        let region =
            Region::create(&format!("onetimer_soak_{logical}"), byte_size).expect("Shm-Region");
        let _ = triton
            .raw()
            .await
            .expect("Backend")
    ...
    let _ = writeln!(
        streams_csv,
        "window,elapsed_s,load,stream,uncovered_permille,aoi_p95_ms,\
    ...
    for window in 0..windows {
        ...
        for r in &reports {
            ...
            let _ = writeln!(
                ...
                r.coverage.aoi_p95_ns / 1_000_000,
                r.emitted,
                r.sent,
                r.client_dropped,
                r.delivered,
