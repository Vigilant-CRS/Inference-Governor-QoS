...
impl RequestDescriptor {
    ...
    /// Wahr, wenn dieser Request von `other` ueberholt werden darf.
    ///
    /// Supersession setzt voraus: gleiche Policy-Erlaubnis, gleiches Modell,
    /// gleicher Scope, kein Stateful-Request und ein tatsaechlich juengerer
    /// Herausforderer. Die Generationszeit entscheidet, nicht die Ankunftszeit —
    /// ein verzoegert eingetroffener aelterer Frame ueberholt nichts.
    #[must_use]
    pub const fn is_superseded_by(&self, other: &Self) -> bool {
        self.queue_policy.allows_supersession()
            ...
            && self.supersession_key.0 == other.supersession_key.0
