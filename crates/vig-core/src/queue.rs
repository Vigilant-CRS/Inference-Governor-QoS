...
impl ModelQueue {
    ...
    pub fn push(&mut self, incoming: RequestDescriptor) -> PushOutcome {
        ...
        if self.config.policy.allows_supersession() && !incoming.stateful {
            ...
            // Scope und Alter getrennt: `scope_of` hat den wirksamen Scope
            // bereits nach Policy normalisiert. Wuerde der Altersvergleich den
            // Key erneut pruefen, haette die Normalisierung keine Wirkung.
            let outranked = self
                .entries
                .iter()
                .any(|queued| self.scope_of(queued) == scope && incoming.is_outdated_by(queued));
            if outranked {
                return PushOutcome {
            ...
            self.entries.retain_reporting(
                |queued| {
                    ...
                    !(same_scope && queued.is_outdated_by(&incoming))
                },
                |queued| {
