...
use vig_protocol_oip::inference::{InferParameter, ModelInferRequest, ModelMetadataRequest};
use vig_protocol_oip::params::{P_CLASS, P_DEADLINE_US, P_MAX_AGE_US};
...
/// Daten liegen — Regionsname, Offset, Groesse. Vigilant reicht diese Angaben
...
async fn client_parameters_are_honoured_and_validated() {
    ...
    // Ein unbekannter vig_-Parameter ist ein Fehler, kein Hinweis.
    let mut bad = self::request("detector", 2);
    bad.parameters.insert(
        "vig_deadline_ms".to_owned(),
        InferParameter {
...
async fn report_proxy_overhead_on_the_grpc_copy_path() {
    println!("\n  Backend | direkt      | ueber Vigilant | Zusatz     | relativ");
    println!("  --------|-------------|----------------|------------|--------");
