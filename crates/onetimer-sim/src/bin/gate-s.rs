...
//!
//! Der Simulator ist die **Best-Case-Welt** fuer OneTimer: keine
...
use onetimer_sim::harness::{RunResult, baseline, onetimer, run};
use onetimer_sim::scenario::{self, Scenario};
...
impl Point {
    ...
    fn alive(&self) -> bool {
        self.onetimer_valid.saturating_mul(100) >= self.baseline_best.valid.saturating_mul(95)
    }
}

struct Point {
    load: u64,
    onetimer_uncovered: u64,
    onetimer_stale: u64,
    onetimer_valid: u64,
...
fn measure(scenario: &Scenario) -> Vec<Point> {
    ...
    for load in LOADS {
        ...
        for seed in SEEDS {
            let r = run(
                &scaled,
                onetimer(&scaled, onetimer_core::profile::SafetyMargin::DEFAULT),
                "onetimer".to_owned(),
                seed,
            );
        ...
        points.push(Point {
            load,
            onetimer_uncovered: median(ot_unc),
            onetimer_stale: median(ot_stale),
            onetimer_valid: median(ot_valid),
...
fn ratio(baseline_value: u64, onetimer_value: u64) -> String {
    if onetimer_value == 0 {
    ...
    format!("{:.2}x", baseline_value as f64 / onetimer_value as f64)
}
...
fn section(out: &mut String, scenario: &Scenario, points: &[Point]) {
    ...
    for p in points {
        ...
        let _ = writeln!(
            ...
            "| {} % | {} ‰ | {} ‰ (d={}) | **{}** | {} ‰ | {} ‰ | **{}** | {} / {}{} |",
            p.load / 10,
            p.onetimer_uncovered,
            b.uncovered,
            b.depth,
            ratio(b.uncovered, p.onetimer_uncovered),
            p.onetimer_stale,
            b.stale,
            ratio(b.stale, p.onetimer_stale),
            p.onetimer_valid,
            b.valid,
            if p.alive() { "" } else { " ⚠" },
        );
...
fn verdict(points: &[Point]) -> (bool, String) {
    ...
    let coverage_ok = overload.iter().any(|p| {
        p.onetimer_uncovered > 0
            && p.baseline_best.uncovered >= p.onetimer_uncovered.saturating_mul(2)
    }) || overload
        .iter()
        .any(|p| p.onetimer_uncovered == 0 && p.baseline_best.uncovered > 0);

    let stale_ok = overload.iter().any(|p| {
        p.baseline_best.stale > 0
            && p.onetimer_stale.saturating_mul(10) <= p.baseline_best.stale.saturating_mul(7)
    });
...
fn main() {
    ...
    let _ = writeln!(
        ...
        "\n> Simulierte Falsifikation nach ADR-0001. **Kein Messwert gegen echte \
         Hardware.** Der Simulator ist die Best-Case-Welt fuer OneTimer: keine \
         Proxy-Kosten, keine zweite Backend-Queue, keine Profilfehler. Ein Effekt, \
         der hier nicht gross ist, kann real nur kleiner werden. Gate S kann die \
         Produkthypothese daher **falsifizieren, aber nicht bestaetigen** — \
