...
use onetimer_core::ids::MAX_MODELS;
...
use onetimer_core::request::RequestState;
use onetimer_core::scheduler::{Action, ActionSink, Event, Scheduler};
...
#[derive(Debug)]
pub enum Governor {
    /// Der OneTimer-Scheduler.
    OneTimer(Box<Scheduler>),
...
impl Governor {
    fn on_event(&mut self, now: Instant, event: Event, sink: &mut Actions) {
        match self {
            Self::OneTimer(s) => s.on_event(now, event, sink),
    ...
    fn metrics(&self) -> &Metrics {
        match self {
            Self::OneTimer(s) => s.metrics(),
            Self::Baseline(s) => s.metrics(),
        }
    }
}

/// Baut den OneTimer-Governor fuer ein Szenario.
///
/// # Panics
///
...
#[must_use]
pub fn onetimer(scenario: &Scenario, margin: SafetyMargin) -> Governor {
    let contracts: ArrayVec<ModelContract, MAX_MODELS> = scenario.onetimer_contracts();
    ...
    Governor::OneTimer(Box::new(scheduler))
}
...
pub fn run(scenario: &Scenario, mut governor: Governor, label: String, seed: u64) -> RunResult {
    ...
    let contracts = scenario.onetimer_contracts();
