...
const HORIZON_US: u64 = DEFAULT_HORIZON.as_nanos() / 1_000;
...
fn the_bounds_are_the_formulas_of_the_analysis() {
    let p = base();
    // Δ = δ + 2J + D
    assert_eq!(delivery_bound_us(&p), 1_000 + 4_000 + 33_000);
    // max(0, T + 2J + Δ − A)
    assert_eq!(gap_bound_us(&p), Some((33_000 + 4_000 + 38_000) - 66_000));
...
fn every_assumption_is_checked() {
    ...
    let mut p = base();
    p.deadline_us = 13_000; // Ĉ + 2J = 14 ms
    ...
    assert_eq!(check(&p), Err(Violation::BackgroundBeyondHorizon));

    let mut p = base();
    p.jitter_us = 3_000;
    p.deadline_us = 37_000; // D + 2J = 43 >= T + Ĉ = 43
    p.max_age_us = 80_000;
...
///
/// Frame 4 kommt `+J`, Frame 5 `−J` — Frame 5 trifft `2J` vor seiner
/// Prognose ein. Genau als Frame 4 fertig wird, liegt ein Hintergrundauftrag
/// bereit, geplant so lang, dass er mit der Prognose von Frame 5 gerade noch
/// vereinbar ist: `s + b + Ĉ = e₅ + D`. Frame 5 wartet bis `e₅ + D − Ĉ`, laeuft
/// `Ĉ` und wird bei `e₅ + D` fertig — sein Alter ist dann genau
/// `δ + 2J + D = Δ`.
#[test]
fn the_bound_is_reached_exactly() {
    ...
    assert_eq!(
        ...
        delivery_bound_us(&p),
        "die Schranke δ + 2J + D wird auf die Mikrosekunde erreicht"
    );
}
...
/// (A6) `T ≤ H` oder Hintergrund ≤ `H`. Liegt die naechste geschuetzte
/// Ankunft ausserhalb des Horizonts, sieht der Look-ahead sie nicht — und
/// laesst Arbeit starten, die ueber sie hinwegreicht.
#[test]
fn an_arrival_beyond_the_horizon_is_not_protected() {
    let mut p = base();
    p.period_us = 120_000;
    p.jitter_us = 0;
    p.jitter = JitterPattern::None;
    p.deadline_us = 10_000; // = Ĉ
    ...
    assert!(
        outcome.worst_age_us > delivery_bound_us(&p),
        "ohne (A6) muss die Schranke reissen: {outcome:?}"
    );
}
...
fn when_a_frame_can_still_wait_at_the_next_arrival_it_is_superseded() {
    ...
    p.deadline_us = 37_000; // D + 2J = 43 = T + Ĉ
    ...
    // bis e₄ + D − Ĉ warten laesst — und das ist genau a₅.
