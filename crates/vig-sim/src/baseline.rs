...
//!
//! Spec 19.1 ist eindeutig: *„Vigilant darf nicht gegen einen absichtlich
...
use vig_core::ids::{MAX_MODELS, ModelIdx, RequestId, SlotIdx};
use vig_core::metrics::Metrics;
use vig_core::model::ModelContract;
use vig_core::profile::SafetyMargin;
use vig_core::request::{Criticality, RequestDescriptor, RequestState};
use vig_core::scheduler::{Action, ActionSink, Event};
use vig_core::slots::SlotSet;
use vig_core::{Instant, VariantIdx};
...
impl BaselineScheduler {
    ...
    pub fn on_event<S: ActionSink>(&mut self, now: Instant, event: Event, sink: &mut S) {
        match event {
            ...
            Event::Cancel { request } => {
                // Auch die Baseline soll zurueckgezogene Arbeit nicht mehr
                // ausfuehren — sonst waere der Vergleich zu ihren Gunsten
                // verzerrt statt zu ihren Ungunsten.
                for queue in &mut self.queues {
                    if let Some(index) = queue.iter().position(|d| d.id == request) {
                        queue.remove(index);
    ...
    fn complete<S: ActionSink>(
        ...
        // Dieselbe Bewertung wie bei Vigilant: war das Ergebnis bei
